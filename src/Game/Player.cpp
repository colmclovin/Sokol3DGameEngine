...
void PlayerController::Spawn(const hmm_vec3 &pos) {
    ...
    // ADDED: Player collider (sphere for smooth movement)
    ...
    // ADDED: Selectable component (for editor selection & visualization)
    Selectable playerSelectable;
    playerSelectable.name = "Player";
    playerSelectable.volumeType = SelectionVolumeType::Sphere;
    playerSelectable.boundingSphereRadius = 1.0f; // Larger selection radius for easier clicking
    playerSelectable.boundingRadius = 1.0f; // Backward compatibility
    playerSelectable.showWireframe = true;
    playerSelectable.canBeSelected = true;
    ecs_.AddSelectable(entityId_, playerSelectable);
    ...
    printf("Player spawned at (%.2f, %.2f, %.2f) with collider radius=%.2f and selectable sphere radius=%.2f\n",
            pos.X, pos.Y, pos.Z, playerCollider.radius, playerSelectable.boundingSphereRadius);
}
